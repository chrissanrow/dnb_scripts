...
countries = set()
for file in glob.glob('dnb_viz_*.json'):
    with open(file) as f:
        data = json.load(f)
        countries.update(entry['country'] for entry in data)

country_to_coords = {}
for country in countries:
    coords = cache.get(country) or get_coords(country)
    if coords:
        country_to_coords[country] = coords
        print(f"{country}: {coords['lat']}, {coords['lng']}")
    else:
...
for file in glob.glob('dnb_viz_*.json'):
    ...
    for entry in data:
        ...
        if country == "Unknown" or country == "Worldwide" or ("&" in country):
            data.remove(entry)
            continue
